...
impl App {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> Action {
        ...
        if self.is_filtering_recent && self.modal == Modal::None {
            return self.handle_key_recent_filter(&key);
        }

        // 1. Priority modals
        if self.modal != Modal::None {
            if matches!(self.modal, Modal::FirstRun(_)) {
                return self.handle_key_first_run(&key);
            }
            ...
            return match &self.modal {
                ...
                Modal::ConfirmDelete(rel) => {
                    let rel = rel.clone();
                    self.handle_key_confirm_delete(&rel, &key)
                }
                Modal::Settings => self.handle_key_settings(&key),
                Modal::Files => self.handle_key_files(&key),
                Modal::Filters => self.handle_key_filters(&key),
                Modal::DryRun => self.handle_key_dry_run(&key),
                Modal::Help => self.handle_key_help(&key),
                Modal::HistoryDetails(run_idx) => {
                    let idx = *run_idx;
                    self.handle_key_history_details(idx, &key)
                }
                Modal::FirstRun(_) | Modal::None => Action::None,
            };
        }

        // 2. Global Dashboard shortcuts
        self.handle_key_dashboard(&key)
    }

    fn handle_key_recent_filter(&mut self, key: &KeyEvent) -> Action {
        match key.code {
            KeyCode::Esc => {
    ...
    fn handle_key_first_run(&mut self, key: &KeyEvent) -> Action {
        if let Modal::FirstRun(ref mut state) = self.modal {
            ...
            match state.step {
                ...
                FirstRunStep::GoogleCredentials => match key.code {
                    ...
                    KeyCode::Home => {
                        match state.active_field {
                            FirstRunField::ClientIdInput => state.client_id_cursor = 0,
                            FirstRunField::ClientSecretInput => state.client_secret_cursor = 0,
                            _ => {}
                        }
                    }
                    KeyCode::End => {
                        match state.active_field {
                            FirstRunField::ClientIdInput => state.client_id_cursor = state.client_id.chars().count(),
                            FirstRunField::ClientSecretInput => state.client_secret_cursor = state.client_secret.chars().count(),
                            _ => {}
                        }
                    }
                    KeyCode::Backspace => {
    ...
    fn handle_key_menu(&mut self, key: &KeyEvent) -> Action {
        match key.code {
            KeyCode::Esc => {
                self.modal = Modal::None;
            }
    ...
    fn handle_key_settings(&mut self, key: &KeyEvent) -> Action {
        if self.is_editing_setting() {
    ...
    fn handle_key_files(&mut self, key: &KeyEvent) -> Action {
        match key.code {
            KeyCode::Esc => {
                self.modal = Modal::None;
            }
    ...
    fn handle_key_filters(&mut self, key: &KeyEvent) -> Action {
        if self.is_editing_filter() {
            match key.code {
                KeyCode::Enter => {
                    self.commit_filter_edit();
    ...
    fn handle_key_dry_run(&mut self, key: &KeyEvent) -> Action {
        match key.code {
            KeyCode::Esc | KeyCode::Char('q') => {
                self.modal = Modal::None;
            }
    ...
    fn handle_key_help(&mut self, key: &KeyEvent) -> Action {
        match key.code {
    ...
    fn handle_key_history_details(&mut self, run_idx: usize, key: &KeyEvent) -> Action {
        let (has_errors, files_len) = self.past_runs.get(run_idx)
        ...
        match key.code {
            ...
            KeyCode::Char('y') | KeyCode::Char('c') if has_errors || files_len > 0 => {
                self.copy_history_errors(run_idx);
            }
            _ => {}
        }
        Action::None
    ...
    fn handle_key_dashboard(&mut self, key: &KeyEvent) -> Action {
        match key.code {
