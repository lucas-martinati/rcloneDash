...
export function updateRuns(runs) {
  ...
  for (let i = 0; i < runs.length; i++) {
    ...
    let statusTxt =
      r.status === 'success' ? '✓ Réussie' : r.status === 'failed' ? '✗ Erreur' : '⟳ En cours';
    ...
    html +=
      ...
      i +
      ')" title="Afficher le détail de cette sync">' +
      '<td style="color:var(--muted)"><span class="chev">▶</span>' +
      fmtDT(r.start) +
      '</td>' +
      '<td><span class="pill ' +
      statusCls +
    ...
    if (r.error_logs && r.error_logs.length > 0) {
      ...
      detHtml +=
        ...
        '<button class="iconbtn" onclick="copyErrorLogs(' +
        i +
        ', this); event.stopPropagation();">Copier</button>' +
        '</div>' +
        '<div class="error-log-box">' +
        colorizeLog(r.error_logs.join('\n')) +
        '</div>';
    }

    if (r.synced_files && r.synced_files.length > 0) {
      detHtml +=
        '<div style="font-weight:600;margin-top:10px">Fichiers affectés (' +
        r.synced_files.length +
        ') :</div>';
      ...
      for (let j = 0; j < Math.min(r.synced_files.length, 50); j++) {
        ...
        let actionCall =
          "openFile('" + pathArg + "', " + (cls === 'deleted' ? 'true' : 'false') + ', event)';
        let fSize = fmtSize(f.size);
        detHtml +=
          '<div class="run-details-file recent-item file-link ' +
          cls +
      ...
      if (r.synced_files.length > 50) {
        detHtml +=
          '<div style="padding:4px;color:var(--muted)">+ ' +
          (r.synced_files.length - 50) +
          ' autres fichiers…</div>';
      }
      detHtml += '</div>';
    } else {
      detHtml +=
        '<div style="color:var(--faint);margin-top:10px">Aucun fichier n\'a changé durant cette sync.</div>';
    }
    detHtml += '</div>';
    ...
    html +=
      ...
      i +
      '" style="display:' +
      (isOpen ? 'table-row' : 'none') +
      '"><td colspan="7" style="padding:0 10px 8px; max-width:0; white-space:normal;">' +
      detHtml +
      '</td></tr>';
  }
  tb.innerHTML = html;
