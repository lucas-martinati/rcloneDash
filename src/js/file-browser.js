...
export function openTreeModal() {
  ...
  loadTree('');
  setTimeout(function () { let s = document.getElementById('fm-search'); if (s) s.focus(); }, 80);
}
...
export function fmRenderCrumbs() {
  ...
  let html = '<button class="fm-crumb root" onclick="loadTree(\'\')" title="Racine synchronisée">'
...
export async function fmRunSearch() {
  let q = _fm.filter;
  try {
    let r = await fetch('/api/search?dir=' + encodeURIComponent(_fm.dir) + '&q=' + encodeURIComponent(q));
    let d = await r.json();
...
export function fmRow(item, i) {
  ...
  let delBtn = '<button class="fm-act danger" onclick="event.stopPropagation(); openDeleteModal(\'' + pathArg + '\')" title="Supprimer localement…">'
...
export function fmFooter() {
  ...
  if (_fm.recursive && _fm.filter.length >= 2) {
    ...
    let right = (n ? n : 'Aucun') + ' résultat' + (n > 1 ? 's' : '') + (_fm.truncated ? ' (limité)' : '');
