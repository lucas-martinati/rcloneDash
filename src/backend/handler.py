...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    # ── Dispatch ──────────────────────────────────────────────────────────
    # Les routes sont déclarées dans _GET_ROUTES / _POST_ROUTES en bas de
    ...
    def _q(self, name, default=""):
        """Valeur d'un paramètre de query string (première occurrence)."""
        return parse_qs(urlparse(self.path).query).get(name, [default])[0]

    def _post_json(self):
        """Corps de requête POST décodé en dict JSON (vide si absent)."""
    ...
    def _api_dryrun(self):
        try:
            ...
            cmd = ["rclone", "bisync", config.REMOTE, config.GD_DIR, "--dry-run",
                   "-v", "--tpslimit", "8", "--filter-from", config.FILTERS_PATH]
    ...
    #  Endpoints POST
    # ═══════════════════════════════════════════════════════════════════════
    def _api_filters_save(self):
        try:
            content = self._post_json().get("content", "")
            with open(config.FILTERS_PATH, "w") as f:
                f.write(content)
            self._json({"ok": True})
        except Exception as e:
    ...
    def _api_delete(self):
        ...
        try:
            tp = self._safe_local(self._post_json().get("path", ""))
            if not tp:
                self._json({"ok": False, "error": "Chemin invalide"})
                return
            freed = 0
    ...
    # ═══════════════════════════════════════════════════════════════════════
    #  Fichiers statiques + garde-fous chemin + réponses
    # ═══════════════════════════════════════════════════════════════════════
    def _serve_static(self, p):
        if p in ("/", "/index.html"):
            self._file(os.path.join(self._d, "index.html"), "text/html;charset=utf-8")
        elif p == "/style.css":
            self._file(os.path.join(self._d, "style.css"), "text/css;charset=utf-8")
        elif p == "/app.js":
            self._file(os.path.join(self._d, "app.js"), "application/javascript;charset=utf-8")
    ...
    _GET_ROUTES = {
        ...
        "/api/bwlimit_save": _api_bwlimit_save,
        "/api/dryrun": _api_dryrun,
        "/api/tree": _api_tree,
        "/api/search": _api_search,
        "/api/filters": _api_filters,
        "/api/filters_add": _api_filters_add,
        "/api/open": _api_open,
        "/api/delete_preview": _api_delete_preview,
        "/api/drive_check": _api_drive_check,
        "/api/filters_remove": _api_filters_remove,
        "/api/match_rules": _api_match_rules,
        "/api/rule_impact": _api_rule_impact,
    }
    _POST_ROUTES = {
        "/api/filters_save": _api_filters_save,
        "/api/delete": _api_delete,
        "/api/drive_delete": _api_drive_delete,
        "/api/rule_delete": _api_rule_delete,
    }
