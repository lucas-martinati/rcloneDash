...
export function openDeleteModal(path) {
  ...
  document.getElementById('del-summary').innerHTML =
    '<div class="del-loading">Analyse du contenu…</div>';
  ...
  fetch('/api/delete_preview?path=' + encodeURIComponent(path))
    .then(function (r) {
      return r.json();
    })
    .then(function (d) {
      ...
      let banner = d.ignored
        ? '<div class="del-banner ok">' +
          ICO_CHECK +
          "<div><b>Exclu de la synchronisation.</b> La suppression locale ne sera pas propagée au Drive : tu libères seulement de l'espace sur ce PC.</div></div>"
        : '<div class="del-banner danger">' +
          ICO_WARN +
          "<div><b>Cet élément n'est pas exclu de la synchronisation.</b> Le supprimer localement l'effacera aussi du Drive au prochain bisync. Exclus-le d'abord pour conserver la copie cloud.</div></div>";
      let noun = d.count > 1 ? 'fichiers' : 'fichier';
      banner +=
        '<div class="del-count"><span class="del-big">' +
        d.count +
        '</span> ' +
        noun +
        ' · <b>' +
        esc(fmtSize(d.size) || '0 o') +
        '</b> à supprimer localement' +
      ...
      document.getElementById('del-drive').innerHTML =
        '<button class="btn btn-g del-drive-btn" onclick="runDriveCheck()">' +
        ICO_CLOUD +
        ' Comparer avec le Drive (rclone check)</button>';
      document.getElementById('del-confirm').disabled = false;
    })
    .catch(function () {
      document.getElementById('del-summary').innerHTML =
        '<div class="del-banner danger">Serveur injoignable</div>';
    });
}
...
export function runDriveCheck() {
  ...
  el.innerHTML =
    '<div class="del-drive-load"><span class="fm-spin"></span> Comparaison avec le Drive en cours…</div>';
  fetch('/api/drive_check?path=' + encodeURIComponent(_del.path))
    .then(function (r) {
      return r.json();
    })
    .then(function (d) {
      if (!d.ok) {
        el.innerHTML =
          '<div class="del-banner warn">' +
          ICO_WARN +
          '<div>Comparaison impossible : ' +
          esc(d.error || '') +
          '</div></div>';
        return;
      }
      if (!d.exists) {
        el.innerHTML =
          '<div class="del-banner danger">' +
          ICO_WARN +
          '<div><b>Absent du Drive.</b> Aucune copie cloud détectée : supprimer localement perdrait définitivement ces données.</div></div>';
        return;
      }
      let c = d.counts;
      if (d.fully_backed) {
        el.innerHTML =
          '<div class="del-banner ok">' +
          ICO_CHECK +
          '<div><b>Intégralement présent sur le Drive.</b> ' +
          c.identical +
          ' fichier(s) identiques' +
          (c.drive_only ? ' · ' + c.drive_only + ' en plus côté Drive' : '') +
          '. Suppression locale sans perte.</div></div>';
        return;
      }
      let lines = '';
      if (c.local_only)
        lines +=
          '<div class="dc-line danger">' +
          c.local_only +
          ' fichier(s) uniquement en local — seraient perdus</div>';
      if (c.differ)
        lines +=
          '<div class="dc-line warn">' +
          c.differ +
          ' fichier(s) différents de la version Drive</div>';
      ...
      el.innerHTML =
        '<div class="del-banner warn">' +
        ICO_WARN +
        '<div><b>Différences détectées.</b> Certains fichiers locaux ne sont pas (ou pas à jour) sur le Drive.</div></div>' +
        '<div class="dc-summary">' +
        lines +
        '</div>' +
        (det ? '<div class="dc-detail">' + det + '</div>' : '');
    })
    .catch(function () {
