...
export function ignorePath(path, isDir) {
...
export function openExcludeModal(path, isDir) {
  ...
  fetch('/api/delete_preview?path=' + encodeURIComponent(path))
    ...
    .then(function (d) {
      if (!d.ok) {
        document.getElementById('exc-summary').innerHTML =
          '<div class="del-count">La règle <span class="mono">' +
          esc(excRule()) +
          '</span> sera ajoutée.</div>';
        return;
      }
      let noun = d.count > 1 ? 'fichiers' : 'fichier';
      document.getElementById('exc-summary').innerHTML =
        '<div class="del-count">Concerne <span class="del-big2">' +
        d.count +
        '</span> ' +
        noun +
        ' · <b>' +
        ...
        '</div>' +
        '<div class="exc-rule">Règle ajoutée : <span class="mono">' +
        esc(excRule()) +
        '</span></div>';
      let fl = '';
    ...
    .catch(function () {
      document.getElementById('exc-summary').innerHTML =
        '<div class="del-count">Serveur injoignable pour l\'aperçu.</div>';
    });
}
...
export function openReincModal(path, rules) {
  ...
  document.getElementById('reinc-summary').innerHTML =
    ...
    '<div><b>' +
    esc(path) +
    "</b> n'est pas exclu par une règle qui lui est propre, mais par " +
    (rules.length > 1 ? 'ces motifs généraux' : 'ce motif général') +
    ' :</div></div>';
  let html = '';
